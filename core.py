...
class VkTools:
    ...
    @staticmethod
    def change_date_year(bdate:int) -> int:
        user_year = bdate.split('.')[2] if bdate else None
        now = datetime.now().year
        return int(now - user_year)
    ...
    def get_profile_info(self, user_id:int) -> str:
        ...
        result = {'name': (info['first_name'] + ' ' + info['last_name']) if
                  ...
                  'year': self.change_date_year(info.get('bdate')),
                  'relation': info.get('relation')
                  }
        return result

    def search_worksheet(self, params, offset: int) -> None:
        try:
    ...
    def get_photos(self, id: int) -> None:
        try:
